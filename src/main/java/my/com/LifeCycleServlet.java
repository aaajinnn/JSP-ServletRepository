...
public class LifeCycleServlet extends HttpServlet {
	...
	@Override
	public void init() throws ServletException { // 처음한번만 실행
		super.init();
		count = 0;
	...
	public void service(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		out.println("<h1>LifeCycleServlet</h>");
		count++;
		...
		long memory = rt.freeMemory(); // 사용가능한 메모리
		out.println("<h3>현재 가용 메모리 : "+ memory+"</h3>");
		//count 가 10을 초과하면 Garbage Collector 를 호출해보자(메모리 해제)
		if(count>10) {
			System.gc();
			count=0;
