...
public class MemberUpdateEndServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if (id == null || name == null || pw == null || id.trim().isBlank() || name.trim().isBlank()
				|| pw.trim().isBlank()) {
			res.sendRedirect("member/mypage.html");
			return; // 아래를 수행하지 못하도록 return 을 꼭 해주어야함
		}
		...
		try {
			...
			String msg = (n > 0) ? "회원정보 수정 완료" : "회원정보 수정 실패 - 아이디는 수정할 수 없어요.";
